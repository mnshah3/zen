...
class Momentum:
    ...
    def generate(self, con, asof: date) -> list[Signal]:
        ...
        # Identical for every name in the ranking, so the renderer shows it
        # once rather than repeating it per signal.
        shared_against = [
            "Momentum crashes hard at regime turns; the worst drawdowns come "
            "precisely when the ranking looks strongest.",
            "A high formation return may already reflect news now fully priced, "
            "leaving nothing left to capture.",
            "This baseline applies no valuation or quality screen, so an "
            "expensive or deteriorating business can rank highly.",
        ]
        window_label = f"{form_start:%d %b %Y} to {form_end:%d %b %Y}"

        signals = []
        for rank, (symbol, row) in enumerate(picks.iterrows(), start=1):
            top_pct = round(100 * rank / breadth, 1)
            signals.append(Signal(
                ...
                facts={
                    "Rank": f"{rank} of {breadth} (top {top_pct}%)",
                ...
                rationale=[
                    f"Traded {int(row['sessions'])} of the window's sessions at a "
                    f"median Rs {row['turnover_cr']:.1f} cr a day, so a position "
                    "can be exited.",
                ],
                against=shared_against,
            ))
        return signals
