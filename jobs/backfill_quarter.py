...
quarter's parquet. It never rewrites a row it did not fetch.
...
def main(argv=None) -> int:
    ...
    # Quarter end to five months after: the statutory deadline is 45 days, 60
    # for the March quarter, and some companies file revisions later still.
    start = (pe + pd.Timedelta(days=1)).date()
    end = min((pe + pd.Timedelta(days=150)).date(), date.today())
    if legacy:
        sess = _session()
    ...
    out = (pd.concat([held, rows], ignore_index=True)
             .drop_duplicates(subset=["xbrl_url"]))
